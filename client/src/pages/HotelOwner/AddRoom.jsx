        ...
        <form>
            ...
            <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
                ...
                <div>
                    ...
                    <input className='border border-gray-300 mt-1 rounded p-2 w-24' placeholder='0' value={inputs.pricePerNight} onChange={e => setInputs({ ...inputs, pricePerNight: e.target.value })} type="number" />
                </div>
            </div>

            <p className='text-gray-800 mt-4'>Amenities</p>
            <div className='flex flex-col flex-wrap mt-1 text-gray-400 max-w-sm'>
                {Object.keys(inputs.animaties).map((amenity, index) => (
                    <div key={index}>
                        <input type='checkbox' id={`amenities${index + 1}`} checked={inputs.animaties[amenity]} onChange={() => setInputs({ ...inputs, animaties: { ...inputs.animaties, [amenity]: !inputs.animaties[amenity] } })} />
